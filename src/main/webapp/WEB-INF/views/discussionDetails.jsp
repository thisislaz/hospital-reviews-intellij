...
<main>
	<div x-data="{ showDeleteModal: false, deleteCommentId: null }"
		 @delete-comment.window="showDeleteModal = true; deleteCommentId = $event.detail.commentId; deleteDiscussionId = $event.detail.discussionId"
			...
			<div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
				...
				<div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
					<button type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm"
							@click="showDeleteModal = false; $dispatch('confirm-delete', { commentId: deleteCommentId, discussionId: deleteDiscussionId })">
						Delete
					</button>
								...
								<c:choose>
									<c:when test="${not empty sessionScope.userId and comment.author.id == sessionScope.userId}">
										...
										<div class="block md:flex items-center center-container">
											<a type="button" class="block w-full text-center md:mx-2 lg:mx-2 my-2 py-2 md:py-2 md:px-2 px-4 font-semibold rounded dark:dark:bg-violet-400 dark:dark:text-gray-900 duration-300 hover:bg-violet-300" href="/discussion/${ discussion.id }/comment/edit/${ comment.id }" >Edit comment</a>
											<button
													type="button"
													class="whitespace-nowrap w-full py-2 px-4 md:py-2 md:px-2 font-semibold rounded dark:dark:bg-violet-400 dark:dark:text-gray-900 duration-300 hover:bg-violet-300"
													onclick="dispatchDeleteEvent(${comment.id},${discussion.id})">
												Delete comment
											</button>
										</div>
									</c:when>
