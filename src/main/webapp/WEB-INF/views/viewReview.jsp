...
<header>
	...
	<main>
		<section class="dark:bg-gray-900 md:px-24">
				...
				<div class="container flex flex-col justify-center w-full min-h-screen px-6 py-10 mx-auto lg:absolute lg:inset-x-0">
					...
					<div class="flex flex-col md:flex-row md:justify-between">
						<h1 class="text-2xl font-semibold text-gray-800 capitalize lg:text-3xl dark:text-white">
							What @<span class="text-blue-500"><c:out value="${review.author.username}"></c:out>
							</span> <br> said about
							<span class="text-violet-400 block">
							<c:out value="${review.hospital.hospitalName}"></c:out> <br>
							</span>
							in <span class="text-violet-400">
								<c:out value="${review.hospital.city}, ${review.hospital.state}"></c:out>
							</span>
						</h1>
						<!-- right side of top half -->
						<div class="text-white md:flex items-center mt-4 md:mr-32">
							<div class="block md:flex md:inline md:justify-between md:gap-2 items-center center-container">
								<a type="button" class="block md:mx-6 lg:mx-2 my-2 py-2 md:py-2 md:px-2 px-4 font-semibold rounded dark:dark:bg-violet-400 dark:dark:text-gray-900 duration-300 hover:bg-violet-300" href="/review/new?hospitalName=${review.hospital.hospitalName}" >Review this hospital</a>
								<a type="button" class="block py-2 px-4 md:py-2 md:px-2 font-semibold rounded dark:dark:bg-violet-400 dark:dark:text-gray-900 duration-300 hover:bg-violet-300" href="/hospitals/view/${review.hospital.id}">View hospital details</a>
							</div>
						</div>
					</div>
					<!-- area with picture and article alongside it -->
					<div class="mt-10 lg:mt-20 lg:flex lg:items-center">
						<img class="object-cover object-center w-full lg:w-[32rem] rounded-lg h-96" style="background-image: url('https://source.unsplash.com/random/640x480'); background-position: center center; background-blend-mode: multiply; background-size: cover;">
						<div class="mt-8 lg:px-10 lg:mt-0">
							<h1 class="text-2xl font-semibold text-gray-800 lg:w-72 text-violet-400">
								<q>
									<c:out value="${review.title}"></c:out>
								</q>
							</h1>
							<p class="max-w-lg mt-6 text-lg text-gray-500 dark:text-gray-400">
								<q>
									<c:out value="${review.content}"></c:out>
								</q>
							</p>
							<p class="max-w-lg mt-6 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Unit </span><c:out value="${review.unitWorkedAt}"></c:out>
							</p>
							<p class="max-w-lg mt-1 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Patient ratio </span><c:out value="${review.patientRatio}"></c:out>
							</p>
							<p class="max-w-lg mt-1 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Charting system </span><c:out value="${review.chartingSystem}"></c:out>
							</p>
							<p class="max-w-lg mt-1 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Hospital rating </span><c:out value="${review.rating}"></c:out>
							</p>
							<p class="max-w-lg mt-1 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Management rating </span><c:out value="${review.managementRating}"></c:out>
							</p>
							<p class="max-w-lg mt-1 text-xl text-gray-500 dark:text-gray-400">
								<span class="text-white">Would return </span><c:out value="${review.wouldReturn}"></c:out>
							</p>
							<p class="mt-6 text-lg text-violet-400">
								<span class="text-white">@</span><c:out value="${review.author.username}"></c:out>
							</p>
							<h3 class="font-medium text-gray-500">
								<span class="text-white">Posted </span><c:out value="${review.formattedDate}"></c:out>
							</h3>
						</div>
					</div>
				</div>
